...
def construct(
    ...
    if any([not os.path.exists(f) for f in [
        f'{music_graph_dir}/raw_nodes.csv',
        f'{music_graph_dir}/raw_edges.csv',
        f'{label_graph_dir}/raw_nodes.csv',
        f'{label_graph_dir}/raw_edges.csv',
    ]]):
...
def construct_music(mus_nodes,music_graph_dir):
    ...
    mus_nodes.to_csv(f'{music_graph_dir}/raw_nodes.csv',index=False)
    mus_edges.to_csv(f'{music_graph_dir}/raw_edges.csv',index=False)
...
def construct_labels(label_info,label_relations,label_graph_dir):
    ...
    label_nodes.to_csv(f'{label_graph_dir}/raw_nodes.csv',index=False)
    label_edges.to_csv(f'{label_graph_dir}/raw_edges.csv',index=False)